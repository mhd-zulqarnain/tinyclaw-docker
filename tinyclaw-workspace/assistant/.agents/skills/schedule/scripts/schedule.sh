...
Create flags:
  ...
  --sender-id ID      Sender ID (default: tinyclaw-schedule:<label>)
...
Examples:
  ...
  schedule.sh create --cron "*/2 * * * *" --agent assistant --message "..." --channel discord --sender-id "109..." --label gold_rate_2min
...
build_cron_command() {
    local agent="$1" message="$2" channel="$3" sender="$4" sender_id="$5" label="$6"

    # Escape double quotes in the message for JSON safety
    local escaped_message="${message//\"/\\\"}"
    local escaped_sender="${sender//\"/\\\"}"
    local escaped_channel="${channel//\"/\\\"}"
    local escaped_sender_id="${sender_id//\"/\\\"}"
    ...
    # IMPORTANT: In crontab, unescaped '%' is treated as a newline. Ensure every '%' in the stored command is escaped.
    printf '%s' "/bin/bash -c 'QUEUE_INCOMING=\"$QUEUE_INCOMING\"; MSG_ID=\"${label}_\$(date +\\%s)_\$\$\"; printf '\\''{ \"channel\": \"$escaped_channel\", \"sender\": \"$escaped_sender\", \"senderId\": \"$escaped_sender_id\", \"message\": \"@${agent} ${escaped_message}\", \"timestamp\": \\%s000, \"messageId\": \"\\%s\" }'\\'' \"\$(date +\\%s)\" \"\$MSG_ID\" > \"\$QUEUE_INCOMING/\${MSG_ID}.json\"'"
}
...
cmd_create() {
    local cron_expr="" agent="" message="" channel="schedule" sender="Scheduler" sender_id="" label=""
    ...
    # Default sender_id if not provided
    [[ -z "$sender_id" ]] && sender_id="${TAG_PREFIX}:${label}"
    ...
    # Build the cron line
    local cron_cmd
    cron_cmd=$(build_cron_command "$agent" "$message" "$channel" "$sender" "$sender_id" "$label")
